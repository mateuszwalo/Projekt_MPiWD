...
Na świecie występuje wiele zagrożeń, z którymi bo które mają istotny wpływ na bezpieczeństwo ludzi. Można je podzielić na dwie główne kategorie:
...
Niniejsza analiza ma na celu zbadanie danych pochodzących z platformy EMDAT, aby lepiej zrozumieć charakter i skalę zagrożeń, przed którymi stoją ludzie na całym świecie. W szczególności podjęte zostaną próby odpowiedzi na następujące pytania:
...
W dalszych częściach raportu zostaną szczegółowo omówione kolumny zawarte w zbiorze danych, a także przeprowadzone zostaną analizy odpowiadające na wyżej wymienione pytania badawcze. Szczególna uwaga zostanie poświęcona:
...
Z uwagi na fakt że surowe dane są niezbyt przyjazne do dalszej analizy wybierzemy z danych te kolumny które mają dla nas kluczową wartość i oczyścimy je do poziomu istotnych dla nas predyktorów.
...
**Wnioski do wizualizacij:**

-   Znakomitą większość aktualnych zagrożeń jest winą technologicznych! -\> Człowiek powoduje samoistnie katastrofy klimatyczne, wpływając tym też na zagrożenia naturalne, tworząc efekty ekologiczne negatywne dla planety, np. efekt cieplarniany, dziury ozonowe etc.

-   Na podstawie tej wizualizacji firmy ubezpieczeniowe mogłyby dostosować swoją ofertę do ubezpieczeń majątkowych, ustalając konrektene kwoty np. w danym państwie jeśli częścią wustępują tam powodzie niż pożary etc.

-   Świadomość społeczna naszych tego że ludzkie działania przyczyniają się do wyniczania planety może wiele zmienić, co możemy dokładnie zobaczyć na wyresie
...
ggplot(grouped_data, aes(x = Start.Date, y = Count, color = Disaster.Group, group = Disaster.Group)) +
  ...
  labs(
    title = 'Zmiana liczby typów katastrof w czasie',
...
-   Sumarycznie w Chinach dochodzi do największej liczby katastrof na przestrzeni lat, po dokładnym zagłębieniu się w ten temat zauważyłem że jest spowodowane położeniem, przez co narażone są one na wiele niebezpieczeństw, wysoka urbanizacja, aktywność sejsmiczna etc.

-   Istnieje zależność pomiędzy licznością ludności danego państwa a liczbą kataklizmów, popatrzmy, najbardziej zaludnione kraje, USA, Chiny, Indie mają najwięcej katastrof ponieważ przyczynia się do nich człowiek
...
-   Europa
    ...
    -   Możemy wnioskować że mieszkańcy Europy na przestrzeni lat mieli spore problemy z powodziami lecz, aktualnie przez zmiany klimatu najbardziej doskierwają niszczące upały mieszkańcom Europy
...
p <- ggplot(data_filtered, aes(x = reorder(Disaster.Subtype, Cumulative.Deaths), y = Cumulative.Deaths, fill = Disaster.Subtype)) +
  ...
  labs(
    title = "Liczba wszystkich ofiar katastrof - {frame_time}",
    x = "Typ katastrofy",
...
-   bardzo niska wartość p value wskazuje na silną zależność między regionem a typem katastrofy, co potwierdza wcześniej stawiane wnioski przy wykresie bąbelkowym

W przeciwieństwie do ilości ofiar śmiertelnych, typy katastrof są zależne od kontynety, co wydaje się dosyć intuicyjne na podstawie położenia w innych strefach klimatycznych etc.

**Test Kruskala-Wallisa dla ilości zgonów w zależności od kontynentu**
...
**Uczenie nienadzorowane - analiza gównych składowych - PCA**
...
-   Deaths i Damage to główne zmienne, które wyjaśniają intensywność katastrof w ramach PC3.
...
**Najlepszy model jaki udało się stworzyć**
